    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join-room', async ({ roomId, username, userId, role }) => {
            try {
                ...
                if (room) {
                    const existingParticipant = room.participants.find(p => p.socketId === socket.id || (userId && p.userId?.toString() === userId));
        ...
        // Handle code changes
        socket.on('code-change', async ({ roomId, code, userId, language }) => {
            try {
